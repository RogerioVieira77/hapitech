...
export function TopContactsCard({ items, title, subtitle }: { items: TopContactItem[]; title?: string; subtitle?: string }) {
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="icon-container icon-container-sm shrink-0">
              <ThemedIcon src={customIcons.iconMessage} alt="Contatos" className="h-[18px] w-[18px] opacity-95" />
            </div>
            <div>
              <p className="text-[13px] font-semibold text-foreground">{title || "Top Contatos"}</p>
              <p className="text-[11px] text-muted-foreground/85 mt-0.5">{subtitle || "Mensagens recebidas"}</p>
            </div>
          </div>
        </div>
        <div>
          {items.length === 0 ? (
            ...
            items.slice(0, 5).map((item, i) => {
              ...
              const channelLogo = item.channel ? channelLogos[item.channel] : null;
              ...
              return (
                ...
                >
                  ...
                  <div className="relative shrink-0">
                    ...
                    {channelLogo && (
                      <img src={channelLogo} alt="" className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full object-contain bg-background ring-1 ring-background" />
                    )}
                  </div>
